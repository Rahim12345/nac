        ...
        <div class="content m-3">
            ...
            <div class="row">
                ...
                <div class="form-group mb-3 col-md-4">
                    ...
                    <select name="menu_id" id="menu_id" class="form-control @error('link') is-invalid  @enderror">
                        @foreach($parents as $menu)
                            <option value="{{ $menu['id'] }}" {{ $menu['id'] == old('menu_id') ? 'selected' : '' }}>{{ $menu['menu_en'] }}</option>
                        @endforeach
                    </select>
                    @error('menu_id')
                    <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="form-group mb-3 col-md-4">
                    ...
                    <input type="text" class="form-control @error('menu_az') is-invalid  @enderror" name="menu_az" id="menu_az" value="{{ old('menu_az') }}">
                    @error('menu_az')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group mb-3 col-md-4">
                    ...
                    <input type="text" class="form-control @error('menu_en') is-invalid  @enderror" name="menu_en" id="menu_en" value="{{ old('menu_en') }}">
                    @error('menu_en')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
